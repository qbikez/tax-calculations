...
def build_excel_models(all_work_items: List[WorkItem], prs: List[GitPullRequest]) -> Iterable[ExcelRow]:
    ...
    for pr in prs:
        ...
        yield ExcelRow(pr.merge_id, id, obfuscate(title), obfuscate(pr.title), pr.closed_date, pr.url)
    ...
    for wi in all_work_items:
        if len([pr for pr in prs if wi.id in pr.work_item_refs]) == 0:
            ...
            yield ExcelRow("", wi.id, wi.fields["System.Title"], "", wi.fields["Microsoft.VSTS.Common.ClosedDate"], wi.url)
            
def write_header(ws: Worksheet):
    header = ["PR Id", "Task Id", "Task title", "PR title", "Merged Date", "Time", "URL"]
...
def write_excel(rows: Iterable[ExcelRow], month):
    ...
    for idx, row in enumerate(rows):
        ...
        ws.cell(row=row_id, column=7).value = row.url
